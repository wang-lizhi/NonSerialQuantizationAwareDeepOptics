...
def _channel_attention_block(x, kernel_size=3, name="attention_blk"):
    attention_conv = Conv1D(filters=1, kernel_size=kernel_size, strides=1,  # kernel_initializer='he_normal',
...
def get_res_block_u_net(filter_root, depth, output_channel=31, input_size=(512, 512, 3), activation='elu',
    ...
    for cur_net_num in range(net_num):
        ...
        if net_num > 1 and cur_net_num == 0 and extra_upsampling:
            # 对于 RGGB 4通道的模式 由于输入是 128x128x4，需要最后多做一次上采样才能和数据集 256x256 匹配
            x = UpSampling2D(name="middle_res_up_sampling", interpolation="bilinear")(x)
            # 在首个子网络中添加逆光谱输入残差
            # inv_spec_res = inverse_simulated_rgb_channel_camera_spectral_response_function(original_input,
            #                                                                                input_as_rggb=True)
            # inv_spec_res = Conv2D(filter_root, kernel_size=1, padding='same', use_bias=False,
            #                     name="net{}_sub_net_spec_res_conv".format(cur_net_num))(inv_spec_res)
            # inner_block_res_added = Add(name="net{}_add_inverse_hyper_spectral_image_residual".format(cur_net_num))(
            #     [inv_spec_res, x])
            # x = Activation(activation, name="net{}_sub_net_spec_act_res".format(cur_net_num))(x)
