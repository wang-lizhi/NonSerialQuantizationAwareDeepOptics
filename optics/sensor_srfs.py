...
from constants import SRF_GS3_U3_41S4C_BGR_31_CHANNEL_400_700NM, \
    ...
    SRF_MOCK_BGR_31_CHANNEL_400_700NM, SRF_GS3_31S4C_BGR_31_CHANNEL_400_700NM

SRF_BGR_31_CHANNEL_400_700_NM = SRF_GS3_31S4C_BGR_31_CHANNEL_400_700NM
...
def inverse_simulated_gray_channel_camera_spectral_response_function(input_gray_image):
    ...
    gray_channel_expanded = gray_channel_expanded \
                            * tf.reduce_sum(SRF_GRAY_31_CHANNEL_400_700_NM) \
                            / SRF_GRAY_31_CHANNEL_400_700_NM
    return gray_channel_expanded


def inverse_simulated_rgb_channel_camera_spectral_response_function(input_rgb_image,
                                                                    input_as_rggb=False, channel_num=31):
    masked_response_function = SRF_BGR_31_CHANNEL_400_700_NM
    masked_response_function = tf.cast(masked_response_function, dtype=input_rgb_image.dtype)
    green_2_channel = None
    wave_length_sum_response = tf.reduce_sum(masked_response_function, axis=0)  # shape=(25, )
    if input_as_rggb:
        # 若输入为 rggb，则需要拆分为4个通道
    ...
    if input_as_rggb:
        # 对于输入为rggb的情况，将第二个g通道做和绿色通道同样的逆响应变换，然后和原有的绿色通道平均
...
def simulated_rgb_camera_spectral_response_function(hyper_spectral_image):
    ...
    # masked_response_function = SRF_BGR_31_CHANNEL_420_720_NM
    # R 波长最长，取 [2]
    ...
    # 最后的channel已经在被reduce后消除（batch, height, width），此处应使用stack拼接而不是concat
    rgb_image = tf.stack([red_channel, green_channel, blue_channel], axis=-1)
    # 预期 shape: (batch, height, width, 3)
    return rgb_image


def simulated_rgb_camera_spectral_response_function_keep_luminance_difference(hyper_spectral_image):
    channel_num = 31
    masked_response_function = SRF_BGR_31_CHANNEL_400_700_NM
    # R 波长最长，取 [2]
    red_response = hyper_spectral_image * tf.reshape(masked_response_function[2], shape=[1, 1, 1, channel_num])
    red_channel = tf.reduce_sum(red_response, axis=-1)
    # G 取 [1]
    green_response = hyper_spectral_image * tf.reshape(masked_response_function[1], shape=[1, 1, 1, channel_num])
    green_channel = tf.reduce_sum(green_response, axis=-1)
    # B 波长最短，取 [0]
    blue_response = hyper_spectral_image * tf.reshape(masked_response_function[0], shape=[1, 1, 1, channel_num])
    blue_channel = tf.reduce_sum(blue_response, axis=-1)
    # 最后的channel已经在被reduce后消除（batch, height, width），此处应使用stack拼接而不是concat
    rgb_image = tf.stack([red_channel, green_channel, blue_channel], axis=-1)
    # 预期 shape: (batch, height, width, 3)
    return rgb_image / 100
...
def simulated_rggb_bayer_raw_1_channel_response_function(hyper_spectral_image):
    ...
    # 最终 shape 为 (batch, height, width, 1)
...
def simulated_rggb_bayer_separated_4_channel_response_function(hyper_spectral_image):
    ...
    # 保持 RGGB 通道分离，从 height 所在轴起始应用 mask
    ...
    # 最终 shape 为 (batch, height/2, width/2, 4)
    return combined_4_channel_image


dynamic_range_clipping_scale = 1.2


def simulated_dynamic_range_clip(hdr_image):
    image_max = tf.reduce_max(hdr_image, name="image_max")
    image = hdr_image * dynamic_range_clipping_scale
    clipped_image = tf.clip_by_value(image, clip_value_min=0, clip_value_max=image_max)

    tf.summary.image("ClippedImages", clipped_image, max_outputs=3)
...
SRF_MAP = {
    ...
    'rgb-keep-lum-diff': simulated_rgb_camera_spectral_response_function_keep_luminance_difference,
    ...
    'dynamic-range-clip': simulated_dynamic_range_clip
}
...
SRF_OUTPUT_SIZE_LAMBDA = {
    ...
    "gray": lambda p: (p, p, 1),
    "dynamic-range-clip": lambda p: (p, p, 3)
}

SRF_FUNC_NAME = {
    "31S4C-400-700": SRF_GS3_31S4C_BGR_31_CHANNEL_400_700NM,
    "41S4C-400-700": SRF_GS3_U3_41S4C_BGR_31_CHANNEL_400_700NM,
    "41S4C-420-720": SRF_GS3_U3_41S4C_BGR_31_CHANNEL_420_720NM
}