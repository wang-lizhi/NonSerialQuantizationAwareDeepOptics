...
from util.data.data_utils import safe_crop_to_bounding_box, decode_hyper_image_from_png_list, crop_cave_to_5_patches, \
    flat_map_image_self_and_transpose, lighten_image, crop_1024, flat_map_256_patches_from_dataset, decode_hdr

DATASET_PATH = {
    "your_dataset_name": "/path/to/your/dataset",
}
...
def load_cave_datasets(dataset_dir, cache_name="cache", verbose=True):
    ...
    for root, dirs, files in os.walk(dataset_dir):
        for _dir in dirs:
            ...
            if png_cnt != 0:
                if "CAVE" in root:
                    if verbose:
                        Logger.i("[Dataset] Loading <", root, "/", _dir, ">\t (", png_cnt, "\t PNGs).")
                    cave.append(hyper_png_path_list)
                else:
                    if verbose:
                        Logger.w("[Dataset] Ignoring directory <", root, "/", _dir, ">\t (", png_cnt, "\t PNGs).")
            else:
                if verbose:
    ...
    cave_dataset = tf.data.Dataset.from_tensor_slices(cave) \
        .map(decode_hyper_image_from_png_list).cache(cache_name + "-cave")
    return cave_dataset
...
def flat_map_512_overlapped_patches_from_ICVL_dataset(_img):
    flat_map_operation_list = [tf.image.resize(images=_img, size=[512, 512], method=tf.image.ResizeMethod.BILINEAR)]
    third_height = 464
    third_width = 434
    # tf.print("MAT shape=", large_height, large_width)
...
def flat_map_256_overlapped_patches_from_ICVL_dataset(_img):
    flat_map_operation_list = [tf.image.resize(images=_img, size=[256, 256], method=tf.image.ResizeMethod.BILINEAR)]
    # tf.print("MAT shape=", large_height, large_width)
...
def flat_map_768_overlapped_patches_from_ICVL_dataset(_img):
    flat_map_operation_list = [tf.image.resize(images=_img, size=[768, 768], method=tf.image.ResizeMethod.BILINEAR),
...
def flat_map_384_overlapped_patches_from_ICVL_dataset(_img):
    flat_map_operation_list = [tf.image.resize(images=_img, size=[384, 384], method=tf.image.ResizeMethod.BILINEAR)]
    # tf.print("MAT shape=", large_height, large_width)
    for i in range(0, 1392, 348):
        for j in range(0, 1300, 325):
            flat_map_operation_list.append(safe_crop_to_bounding_box(_img, i, j, 384, 384))
    tf.print("Divided into patches:", len(flat_map_operation_list))
    print("Divided into patches:", len(flat_map_operation_list))
    return tf.data.Dataset.from_tensor_slices(flat_map_operation_list)
...
def load_icvl_full_mat_384(dataset_dir, cache_name, verbose=True):
    return load_icvl_full_mat(dataset_dir, verbose=verbose).flat_map(
        flat_map_384_overlapped_patches_from_ICVL_dataset).cache(cache_name)


def load_icvl_full_mat_512_patch_div_for_training_and_validation(dataset_dir, cache_name, verbose=True):
    original_set = load_icvl_full_mat(dataset_dir, verbose=verbose)
    training_set = original_set.flat_map(
        lambda _img: tf.data.Dataset.from_tensor_slices([
            safe_crop_to_bounding_box(_img, 0, 0, 512, 512),
            safe_crop_to_bounding_box(_img, 512, 0, 512, 512),
            safe_crop_to_bounding_box(_img, 512, 512, 512, 512)
        ])
    ).cache(cache_name + "-training")
    validation_set = original_set.flat_map(
        lambda _img: tf.data.Dataset.from_tensor_slices([
            safe_crop_to_bounding_box(_img, 0, 512, 512, 512)
        ])
    ).cache(cache_name + "-validation")
...
def flat_map_512_overlapped_patches_from_harvard_dataset(_img):
    ...
    flat_map_operation_list = [
        tf.image.resize(images=tf.image.crop_to_bounding_box(_img, 0, 0, 1040, 1040),
                        size=[512, 512], method=tf.image.ResizeMethod.BILINEAR),
...
def load_icvl_mat_with_cave_512(dataset_dir, cache_name):
    ...
    cave_ds = load_cave_datasets(dataset_dir=DATASET_PATH["CAVE-16bit-LargeTraining"], cache_name=cave_cache_name)
