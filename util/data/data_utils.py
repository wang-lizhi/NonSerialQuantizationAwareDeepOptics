...
def decode_rgb_8bit_png(file_path):
    ...
    png_image /= 255
    tf.debugging.assert_greater_equal(png_image, 0.0)
    tf.debugging.assert_less_equal(png_image, 1.0)
    return png_image


def decode_grayscale_16bit_png(file_path):
    image = tf.io.read_file(file_path)
    png_image = tf.image.decode_png(contents=image, channels=1, dtype=tf.uint16)  # 光谱灰度图，channel数为1
    tf.print("Decoding 16-bit PNG: max_val=", tf.reduce_max(png_image), "min_val", tf.reduce_min(png_image),
             "<shape=", png_image.shape, ">@", file_path)
    png_image = tf.cast(png_image, tf.float32)
    png_image = tf.divide(png_image, 65535.0)
    tf.print("Decoded & normed PNG: max_val=", tf.reduce_max(png_image), "min_val", tf.reduce_min(png_image))
    tf.debugging.assert_greater_equal(png_image, 0.0)
    tf.debugging.assert_less_equal(png_image, 1.0)
    return png_image


def decode_hdr(file_path):
    image = tf.io.read_file(file_path)
    hdr_image = tfio.experimental.image.decode_hdr(image)
    tf.print("Decoding HDR: <shape=", hdr_image.shape, ">@", file_path)
    hdr_image = tf.cast(hdr_image, tf.float32)
    return hdr_image
...
def decode_hyper_image_from_png_list(image_list):
    ...
    tf.print("Decoded hyper_image: max_val=", tf.reduce_max(hyper_image), "min_val", tf.reduce_min(hyper_image))
    tf.debugging.assert_greater_equal(hyper_image, 0.0)
    tf.debugging.assert_less_equal(hyper_image, 1.0)
    return hyper_image
...
def flat_map_256_patches_from_dataset(_img):
    flat_map_operation_list = [tf.image.resize(images=_img, size=[256, 256],
...
def flat_map_512_patches_from_dataset(_img):
    flat_map_operation_list = [tf.image.resize(images=_img, size=[512, 512],
                                               method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)]
...
def crop_1024(image):
    ...
    # print("crop_1024 cropped:", cropped)
    return cropped
