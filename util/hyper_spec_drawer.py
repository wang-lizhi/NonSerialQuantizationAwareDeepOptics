...
def wavelength_to_rgb(wavelength, gamma=0.8):
    ...
    return: R, G, B values, whose maximum is 1.
    """
...
def tile_hyper_channels_to_rgb_space_patch(hyper_spec_image, row=4, col=8, padding=0, norm=False):
    ...
    return tiled_image
    # return tf.batch_to_space(hyper_spec_image, [8, 4], crops=[[0, 0], [0, 0]])
...
def save_hyper_to_tiled_rgb_file(hyper_image, save_path, with_hyper_tiles=False, with_rgb=True, norm=False):
    ...
    if hyper_image.shape[-1] <= 3:
        print("Directly saving RGB image...")
        encoded_rgb = tf.squeeze(tf.cast(hyper_image * 255, dtype=tf.uint8), axis=0)
        encoded_rgb = tf.image.encode_png(encoded_rgb)
        save_numpy_to_file(save_path + ".png", encoded_rgb.numpy())
        return

    if with_hyper_tiles:
        tiled = tile_hyper_channels_to_rgb_space_patch(hyper_image, padding=3, norm=norm)
        tiled = tf.cast(tiled * 255, dtype=tf.uint8)
        # hyper_img = tf.transpose(hyper_img, perm=[3, 1, 2, 0])  # C H W B
    ...
    if with_rgb:
        ...
        # rgb = simulated_rgb_camera_spectral_response_function_for_visualization(hyper_image)
        rgb = rgb / tf.reduce_max(rgb)
        rgb = tf.squeeze(tf.cast(rgb * 255, dtype=tf.uint8), axis=0)
        rgb = tf.image.encode_png(rgb)
        print("Saving RGB images at ", save_path, "...")
