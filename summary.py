...
class EpochSummaryCallback(tf.keras.callbacks.Callback):
    ...
    def calculate_metrics(ground_truth, predicted_image, summary_id=0, post_recheck=False):
        if post_recheck:
            ...
            summary_hyper_spec_image(image=unpack_reconstruction_from_prediction(predicted_image),
                                     name="%d-PredictedImage" % summary_id)
            summary_hyper_spec_image(image=unpack_element_from_prediction(predicted_image, pos=1),
                                     name="%d-PostReEncode" % summary_id)
            summary_hyper_spec_image(image=unpack_element_from_prediction(predicted_image, pos=2),
                                     name="%d-PreSensor" % summary_id)
        else:
...
def image_normalization(image, given_max=None, given_min=None):
    ...
    return tf.cond(given_max != 0, lambda: image / given_max, lambda: image)


def summary_hyper_spec_image(image, name="InputImage", description=None, with_single_channel=False, norm_all=True,
                             norm_channel=False, impossible_to_be_rgb=False):
    if not impossible_to_be_rgb and image.shape[-1] <= 3:
        tf.summary.image(name=name, data=image, description=description, max_outputs=1)
        return
